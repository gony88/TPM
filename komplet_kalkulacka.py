...
#                  a geometrie drážek.
#                  NOVĚ: Viskozita je určena přímo teplotou 'T_olej_inlet' (Konzervativní přístup).
...
INCLUDE_AREA_REDUCTION = True   # <--- ZDE ZMĚŇTE VOLBU
...
n_motor_end   = 1000.0  # [rpm] Otáčky motoru na konci prokluzu (synchronizace)
n_motor_idle  = 1500.0   # [rpm] Otáčky motoru v PAUZE (volnoběh)
...
def get_cooling_analytical(rpm, T_viscosity_input, geometry):
    ...
    Vstupy:
      ...
      T_viscosity_input : Teplota použitá PRO URČENÍ VISKOZITY.
                          V této verzi kódu je to fixní T_olej_inlet.
    ...
    # Interpolujeme kinematickou viskozitu 'nu' podle zadané teploty.
...
ax5.set_ylabel('Teplota pro viskozitu [°C]', color='purple', fontsize=12, fontweight='bold')
ax5.plot(cas_plot, T_oil_film_log, 'm-', label='Konstantní teplota pro viskozitu', linewidth=2)
ax5.axhline(y=T_olej_inlet, color='gray', linestyle='--', label=f'Teplota Přívodu ({T_olej_inlet}°C)')
ax5.set_title(f'3. Teplota oleje určující viskozitu ({T_olej_inlet}°C)', fontsize=12)
