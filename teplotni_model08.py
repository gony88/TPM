...
#   "STATIC"     = malý průtok
#   "ANALYTIC"   = dynamický výpočet h dle otáček a viskozity oleje
#   "FLOW_LIMIT" = zanedbání hydrodynamiky, použití fixního h dle zadaného průtoku dle rovnice Q = m_dot * c_p * Delta_T
#   "NO_COOLING" = žadné chlazení (h = 0)

CHLAZENI_TYP = "FLOW_LIMIT"   # <--- ZDE PŘEPSATE REŽIM CHLAZENÍ

# ZAPOČÍTANÍ SNÍŽENÉ PLOCHY
# True  = Plocha mezikruží je snížena o drážky
# False = Celá plocha mezikruží
INCLUDE_AREA_REDUCTION = False   # <--- ZDE ZMĚŇIT VOLBU
...
T_olej_inlet = 70.0   # [°C] Teplota oleje na vstupu. 
                      # Ovlivňuje Delta T a viskozitu -> koeficient h.

# --- D. ROZJEZD VE SVAHU  ---

ENABLE_HILL_START = False   # Zapnout simulaci kopce? (True/False)

# Parametry pro ROZJEZD VE SVAHU (pouze pokud je ENABLE_HILL_START = True)
t_hold       = 1.0    # [s]  doba předzáběru před samotným rozjezdem.
n_motor_hold = 1800.0 # [rpm] Otáčky motoru pro předzáběr.
...
n_motor_idle  = 1000.0   # [rpm] Otáčky motoru po sepnutí (ovlivňuje chlazení)
...
# --- F. ČASOVÁNÍ CYKLU ---
n_cyklu = 10           # Počet sekvencí rozběhů
t_zab   = 1.740        # [s] Doba rozjezdu (záběru)
t_pauza = 15.0         # [s] Doba sepnutí
