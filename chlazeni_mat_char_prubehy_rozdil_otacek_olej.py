...
n_motor_start = 2000.0  # [rpm] Otáčky motoru na začátku záběru
n_motor_end   = 2000.0  # [rpm] Otáčky motoru na konci záběru
...
t_zab   = 1.74        # [s] Doba trvání prokluzu (záběru)
...
while t < t_total:
    t_local = t % t_cyklus # Čas v rámci aktuálního cyklu

    # ------------------------------------------------------------------
    # KROK 1: URČENÍ AKTUÁLNÍCH OTÁČEK
    # ------------------------------------------------------------------
    if t_local <= t_zab:
        # FÁZE ZÁBĚRU (Spojka prokluzuje)
        ratio = t_local / t_zab
        
        # A) Otáčky motoru (Absolutní) - Pro mapu momentu a čerpadlo
        rpm_engine_abs = n_motor_start + (n_motor_end - n_motor_start) * ratio
        
        # B) Otáčky prokluzu (Rozdíl) - Pro generování tepla
        rpm_slip = n_slip_start * (1 - ratio)
    else:
        # FÁZE PAUZY (Spojka rozepnuta)
        rpm_engine_abs = n_motor_idle # Motor běží na volnoběh
        rpm_slip = 0.0                # Žádný prokluz = žádné tření

    # ------------------------------------------------------------------
    # KROK 2: VÝPOČET SOUČINITELE PŘESTUPU TEPLA 'h'
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # KROK 3: BILANCE TEPELNÝCH TOKŮ (ZDROJ vs. CHLAZENÍ)
    # ------------------------------------------------------------------
    if t_local <= t_zab:
        # --- JSME V ZÁBĚRU ---
        
        # 1. Zjistíme moment z mapy motoru (dle otáček motoru!)
        real_torque = get_torque_from_rpm(rpm_engine_abs)
        if real_torque < 0: real_torque = 0
        
        # 2. Spočítáme třecí výkon (Moment * Rychlost prokluzu)
        omega_slip = rpm_slip * 2 * np.pi / 60
        power = real_torque * omega_slip
        
        # 3. Přepočet na tepelný tok [W/m2] (Vstup tepla)
        q_gen = (power / n_pairs / S_calc_power) * beta
        
        # 4. Výpočet chladicího toku (Newtonův ochlazovací zákon)
        # Teplo odchází do oleje o teplotě T_olej_inlet (70°C).
        # ratio_groove = chladíme jen v drážkách (protože desky jsou na sobě).
        q_cool = h_current * (T[0] - T_olej_inlet) * ratio_groove
        
        # 5. Výsledný tok do materiálu (Vstup - Výstup)
        q_net = q_gen - q_cool
        ...
        # --- JSME V PAUZE ---
        q_gen = 0.0 # Žádné tření
        
        # Stále chladíme! (Motor běží na volnoběh -> čerpadlo jede)
        q_cool = h_current * (T[0] - T_olej_inlet) * ratio_groove
        q_net = -q_cool # Pouze odebíráme teplo

        current_torque_plot = 0.0
        current_power_plot = 0.0

    # ------------------------------------------------------------------
    # KROK 4: VÝPOČET TEPLOTY (SOLVER)
    # ------------------------------------------------------------------
...
print("\n" + "="*60)
print(f" VÝSLEDKY SIMULACE ({n_cyklu} cyklů)")
print("="*60)
